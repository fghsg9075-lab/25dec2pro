...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      if (loggedInUserStr) {
        ...
        // STRICT ROLE SEPARATION INITIALIZATION
        let initialView: ViewState = 'BOARDS';
        if (user.role === 'ADMIN') {
        ...
        setState(prev => ({ 
          ...prev, user: user, view: initialView, selectedBoard: user.board || null, selectedClass: user.classLevel || null, selectedStream: user.stream || null, language: user.board === 'BSEB' ? 'Hindi' : 'English', showWelcome: !hasSeenWelcome && !!hasAcceptedTerms
        }));
      }
  }, []);
