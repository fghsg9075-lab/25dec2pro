...
interface ContentConfig {
    ...
    ultraPdfLink?: string; // NEW: Ultra PDF Link
...
export const AdminDashboard: React.FC<Props> = ({ onNavigate, settings, onUpdateSettings, onImpersonate, logActivity }) => {
  ...
  useEffect(() => {
      if (!['SYLLABUS_MANAGER', 'CONTENT_PDF', 'CONTENT_VIDEO', 'CONTENT_MCQ', 'CONTENT_TEST', 'CONTENT_NOTES', 'BULK_UPLOAD'].includes(activeTab)) {
          setSelSubject(null);
          // Don't aggressively clear editingChapterId here to preserve state if navigating briefly to other tabs, 
          // but we do want to reset if leaving "content" mode entirely.
          // For now, let's keep it but rely on auto-saving to localStorage or explicit save for persistence.
...
<html lang="en">
  ...
  const loadChapterContent = async (chId: string) => {
      ...
      // 1. Try Local First (Instant Load) - This might be a "draft" or "synced" version
      const stored = localStorage.getItem(key);
      if (stored) {
      ...
      // Only overwrite if cloud has data. If cloud is empty but local has draft, keep local.
      if (isFirebaseConnected) {
          try {
              const cloudData = await getChapterData(key);
              if (cloudData) {
                  // Merge strategy: If local has more recent "draft" timestamp? 
                  // For now, simple strategy: Cloud wins if it exists, to ensure consistency across admins.
                  // BUT, if we just saved locally and it hasn't synced yet, this might overwrite.
                  // Ideally we'd compare timestamps.
                  // Current implementation: Cloud overwrites local view.
                  // To fix "back navigation data loss", we rely on the fact that 'saveChapterContent' writes to localStorage.
  ...
  const saveChapterContent = () => {
      ...
      const newData = {
          ...
          ultraPdfLink: editConfig.ultraPdfLink, // Save Ultra Link
                      ...
                      {activeTab === 'CONTENT_PDF' && (
                          <div className="space-y-6">
                              ...
                              <div>
                                  ...
                                  <div className="flex items-center bg-white border rounded-xl overflow-hidden border-yellow-200">
                                      <div className="bg-yellow-50 p-3"><Link size={16} className="text-yellow-600" /></div>
                                      <input type="text" value={editConfig.ultraPdfLink || ''} onChange={e => setEditConfig({...editConfig, ultraPdfLink: e.target.value})} className="flex-1 p-3 outline-none text-sm" placeholder="https://... (For Ultra Users)" />
                                  </div>
                              </div>
